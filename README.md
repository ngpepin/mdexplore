...
- `--number` / `-n` is mutually exclusive with `--wip` / `-w`. It displays a single gray status line such as `   125 files examined;    100 pdf;     25 png`, updating that line in place with aligned counts. The transient status is trimmed with `...` when needed to stay within the current terminal width, preventing terminal wrapping from turning later increments into additional visual lines. When a match is printed, the status line is cleared; counting then resumes on a fresh in-place status line until the next match or the search completes.
